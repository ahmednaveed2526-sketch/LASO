...
# Attempt to import local modules; if running as a script the package
# resolution may fail in some environments (e.g., editors/linters).
def _safe_import(module_name, attr=None):
    try:
        mod = import_module(module_name)
    except Exception:
        # Ensure current directory (script directory) is on sys.path and retry
        import os
...
TestingReport = _safe_import('testing_report', 'TestingReport')
RatingDebugger = _safe_import('debugger', 'RatingDebugger')
ServiceCategory = _safe_import('models', 'ServiceCategory')
Utils = _safe_import('utils', 'Utils')
# Import RatingSystem via safe loader to avoid resolution issues in some editors
RatingSystem = _safe_import('rating_system', 'RatingSystem')
